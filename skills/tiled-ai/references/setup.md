...
Determine the actual Tiled extension directory from its preferences, existing installation or platform configuration. On a standard Linux installation it is `${XDG_CONFIG_HOME:-$HOME/.config}/tiled/extensions`. Do not apply that Linux path to Windows/macOS or a custom installation. If the location remains uncertain, ask for the directory shown in Tiled Preferences instead of installing into a guessed location.
